...
static void bmcu_start_github_version_check()
{
    ...
    const long long now_ms = static_cast<long long>(wxGetUTCTimeMillis().GetValue());
    ...
    std::thread([]() {
        ...
        url += "?ts=" + std::to_string(static_cast<unsigned long long>(wxGetUTCTimeMillis().GetValue()));
        ...
        if (extra_ams_it != obj->module_vers.end()) {
            ...
            for (std::map<std::string, DevAms*>::const_iterator iter = ams_list.cbegin(); iter != ams_list.cend(); iter++) {
                ...
                if (it == ver_list.end()) {
                    ...
                    if (is_bmcu)
                        ams_name = wxString::Format("BMCU-%d", int(ams_id + 1));
                }
    ...
    : wxPanel(parent,id,pos,size,style,name)
{
     ...
     m_fw_link->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent&) {
         if (!m_fw_url.empty())
             wxGetApp().open_browser_with_warning_dialog(m_fw_url);
     });
