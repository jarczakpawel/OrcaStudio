#include "RuntimeCoreSubprocess.hpp"
#include "RuntimeCoreFrame.hpp"
...
struct RuntimeSubprocess::Impl {
    bp::opstream in;
    bp::ipstream out;
    std::unique_ptr<bp::child> child;
};

RuntimeSubprocess::RuntimeSubprocess() : m_impl(std::make_unique<Impl>()) {}
RuntimeSubprocess::~RuntimeSubprocess() { stop(); }

bool RuntimeSubprocess::start(const LaunchSpec& spec)
{
...
bool RuntimeSubprocess::running() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_impl && m_impl->child && m_impl->child->running();
}

void RuntimeSubprocess::stop()
{
...
nlohmann::json RuntimeSubprocess::request(const std::string& method, const nlohmann::json& payload)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string RuntimeSubprocess::last_error() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_last_error;
}
