...
if (-not $docker) {
    throw 'docker not found. Install Docker Desktop or set SLICER_LINUX_RUNTIME_WSL_ROOTFS_TAR to an existing rootfs tar.'
}

New-Item -ItemType Directory -Force -Path (Split-Path -Parent $OutputTar) | Out-Null

$containerName = "bambu_network-bambu-rootfs-" + [guid]::NewGuid().ToString('N')
try {
